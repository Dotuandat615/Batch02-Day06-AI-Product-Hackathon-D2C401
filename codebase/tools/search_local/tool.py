...
class SearchNearbyInput(BaseModel):
    """Validate và coerce args từ agent trước khi gọi LocationService."""
    query:       str
    lat:         Optional[float] = None
    lng:         Optional[float] = None
    radius_km:   float           = Field(default=1.5, gt=0)
...
def execute(
    ...
    """Chạy tool khi agent gọi qua func(**call.args)."""
    inp = SearchNearbyInput(
    ...
    try:
        ...
        restaurants = svc.get_nearby_restaurants(
            loc, radius_km=inp.radius_km, query=inp.query
        )
